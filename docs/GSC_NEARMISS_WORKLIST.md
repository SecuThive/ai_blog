...
## 전체 키워드 전수 감사 (2026-07-15, 596개 질의)

노출≥1 전체를 글에 매핑해 전수 분류한 결과 — "모든 글을 키워드에 맞춰 업데이트"가 정답이 아님을 확인:

| 구간 | 개수 | 결론 |
|---|---|---|
| 승리 1~7위(노출≥3) | 20 | 이미 상위 — 대부분 무조치. 일부만 제목/CTR 개선 대상 |
| 근접 8~20위(노출≥3) | 55 | 처리 완료. 다수는 이미 cov 100%(과최적화 방지 위해 미개입) |
| 중간 20~40위(노출≥3) | 29 | FAQ로는 안 오름 — 대폭 보강/신규 글 영역(엔진·후속) |
| 먼 40위+(노출≥3) | 39 | 신규 글 영역 |
| 롱테일 노출<3 | 453 | 개별 대응 무의미 |

### Round 3 — 근접대역 진짜 갭(FAQ 부재·짧음) 7편 보강
#429 RAGAS answer_relevancy FAQ / #336 GCP 벡터DB(Vertex) FAQ / 가이드#80 sort·uniq 중복제거 / #275 LoRA vs RAG FAQ / 가이드#75 docker restarting FAQ / 가이드#78 protected branch hook declined 섹션 / 가이드#109 strace vs ltrace FAQ

**미개입 판단**: #718·#639·#640 등은 6000자+·FAQ 완비라 과최적화 방지 위해 그대로 둠. 명백한 오매칭(rsync→SELinux, docker vs jq)은 제외.

### 남은 레버 (별도 작업)
1. **저CTR 상위글 제목 개선** — 1~7위인데 클릭 0인 글(예: "kubernetes 최신 트렌드" 3.17위/노출18/클릭0 #469). 본문이 아니라 **title·메타디스크립션**을 검색의도에 맞게 재작성.
2. **20~40위 콘텐츠 갭 16편** — FAQ로 부족, 섹션 대폭 보강 또는 신규 글. 엔진 content_planner로 넘기거나 후속 라운드.

---

## 저CTR 상위글 제목 최적화 (2026-07-15)

**진단**: 저CTR 상위글 후보 29편을 보니 런북·가이드(#590·#614·#107·DNS 등)는 제목·메타가 이미 우수 — 낮은 CTR은 제목이 아니라 **순위(5~9위, top3 아님)** 탓. 즉 제목보다 순위 문제(근접보강으로 대응).

**실제 제목 문제 = SERP 낭비 대괄호 접두사.** 색인글에서 `[필독]·[실전 가이드]·[완벽 가이드]·[비교 가이드]·[시리즈 N/N]·[RAG 완전 정복 N편]`과 "완전 정복" 노이즈를 걷어내 키워드를 앞세움(키워드 보존, 되돌리기 가능).

- **31편 published 제목 정리 적용**(+ #406 draft 1편은 무해). 예: `[필독] LLM 에이전트 보안…` → `LLM 에이전트 보안…`, `서비스 메시 완전 정복: Istio vs…` → `서비스 메시: Istio vs…`
- 중복 제목·과단축 0 검증. ISR 라이브 반영 확인. 백업 `scripts/title-optimize-backup-2026-07-15.json`.
- 메타디스크립션(excerpt)은 대부분 이미 구체적이라 미개입(과최적화 방지).

**결론**: CTR의 남은 병목은 제목이 아니라 순위 — 근접키워드 보강(완료)과 시간이 답. 저CTR 제목 레버는 여기서 실효 대상 소진.
