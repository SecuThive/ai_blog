...
async function fa(t,c,f){let a=[],o=0,s=1000;for(;;){let q=sb.from(t).select(c);if(f)q=f(q);const{data,error}=await q.range(o,o+s-1);if(error)throw error;a=a.concat(data??[]);if((data?.length??0)<s)break;o+=s;}return a;}
