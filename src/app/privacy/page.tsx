...
const SECTIONS = [
  ['1. 수집하는 정보', '① 뉴스레터: 구독 신청 시 이메일 주소를 수집합니다. ② 댓글: 작성 시 입력한 이름(별명 가능)과 댓글 내용, 도배 방지를 위한 IP 주소의 일방향 해시값을 수집합니다. ③ 문의 폼: 이름, 이메일, 소속(선택), 문의 내용을 수집합니다. ④ 이용 분석·광고: 페이지 방문, 이용 시간, 기기·브라우저 정보와 쿠키·광고 식별자가 처리될 수 있습니다.'],
  ...
  ['3. 보관 기간', '뉴스레터 구독 이메일은 구독 해지 처리 후 관련 법령상 보관 의무가 없는 범위에서 삭제합니다. 댓글은 삭제 요청을 확인한 뒤 처리하며, 문의 내역은 답변과 분쟁 대응에 필요한 기간 동안 보관 후 삭제합니다. 광고·분석 데이터와 쿠키의 보관 기간은 각 제공업체(Google·Vercel 등)의 정책과 운영 설정을 따릅니다.'],
  ...
  ['7. AI 도구 및 콘텐츠 피드백', '자료 조사, 콘텐츠 구조화와 초안 작성 과정에서 AI 도구를 보조적으로 활용할 수 있으며, 최종 콘텐츠는 편집 검토 후 공개됩니다. 사용자가 보낸 오류 제보와 콘텐츠 피드백은 해당 글의 확인·정정과 사이트 품질 개선을 위해 검토할 수 있으며, 자체 추천 모델 학습에는 사용하지 않습니다. 별도 동의 없이 개인 식별 정보를 공개하지 않습니다.'],
