...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const title = locale === 'en' ? 'Engineer guides' : '엔지니어 가이드';
  const description = locale === 'en'
    ? 'Technical references for working engineers — Linux, Docker, Git, networking, security, and more.'
    : 'Linux, Docker, Git, 네트워킹, 보안 등 실무 엔지니어를 위한 기술 레퍼런스 모음.';
  const url = locale === 'en' ? `${SITE_URL}/en/engineer` : `${SITE_URL}/engineer`;
  return {
    title,
    description,
    keywords: 'Linux, Docker, Git, networking, security, cloud, databases, engineer guides',
    alternates: {
      canonical: url,
      languages: {
        ko: `${SITE_URL}/engineer`,
        en: `${SITE_URL}/en/engineer`,
        'x-default': `${SITE_URL}/engineer`,
      },
    },
    openGraph: { title, description, url, type: 'website', locale: locale === 'en' ? 'en_US' : 'ko_KR' },
  };
}
